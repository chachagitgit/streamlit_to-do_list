...
def mark_complete(todo_list):
    ...
    if sorted_tasks:
        task_number = st.number_input("Enter the number of the task to mark as complete", min_value=1, max_value=len(sorted_tasks))
...
def remove_task(todo_list):
    sorted_tasks = view_tasks(todo_list)
    if sorted_tasks:
        task_number = st.number_input("Enter the number of the task to remove", min_value=1, max_value=len(sorted_tasks))
        if 1 <= task_number <= len(sorted_tasks):
